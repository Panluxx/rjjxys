...
class TestLogin(BasePage):
    ...
    def test_login(self, open_client):
        ...
        if self.exists(get_path('清空密码'), img_doc='密码输入框'):
            ...
            # 密码输入框
            self.touch(get_path('输入密码'), img_doc='输入密码')
        self.text(password, img_doc=f'输入用户名{password}')
        # 记住密码
        if self.exists(get_path('未勾选'), img_doc='记住密码未勾选'):
            self.touch(get_path('未勾选'), img_doc='勾选密码')
        # 勾选协议
        if self.exists(get_path('未勾选'), img_doc='记住密码未勾选'):
            self.touch(get_path('未勾选'), img_doc='勾选协议')
        # 登录
